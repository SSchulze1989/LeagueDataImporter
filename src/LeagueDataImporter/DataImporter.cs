...
public class DataImporter : IDisposable
{
    ...
    public async Task<ScheduleEntity> ImportSeasonSchedule(SeasonEntity season, ScheduleDataDTO scheduleData)
    {
        ScheduleEntity schedule = await dbContext.Schedules
            ...
            .Include(x => x.Events)
                .ThenInclude(x => x.SimSessionDetails)
            .Where(x => x.LeagueId == LeagueId)
    ...
    public async Task ImportEventResults(EventEntity @event, SessionResultsDTO sessionResults, LeagueMemberEntity[] members, TeamEntity[] teams)
    {
        ...
        if (sessionResults.SessionDetails is not null)
        {
            var detailsEntity = @event.SimSessionDetails.FirstOrDefault();
            if (detailsEntity is null)
            {
                detailsEntity = new();
                @event.SimSessionDetails.Add(detailsEntity);
            }
            detailsEntity = MapSessionDetailsDataToEntity(sessionResults.SessionDetails, detailsEntity);
            await dbContext.SaveChangesAsync();
        }
    }
    ...
    private static IRSimSessionDetailsEntity MapSessionDetailsDataToEntity(SimSessionDetailsDTO sessionDetails, IRSimSessionDetailsEntity detailsEntity)
    {
        detailsEntity.IRSubsessionId = sessionDetails.IRSubsessionId;
        detailsEntity.IRSeasonId = sessionDetails.IRSeasonId;
        detailsEntity.IRSeasonName = sessionDetails.IRSeasonName;
        detailsEntity.IRSeasonYear = sessionDetails.IRSeasonYear;
        detailsEntity.IRSeasonQuarter = sessionDetails.IRSeasonQuarter;
        detailsEntity.IRRaceWeek = sessionDetails.IRRaceWeek;
        detailsEntity.IRSessionId = sessionDetails.IRSessionId;
        detailsEntity.LicenseCategory = sessionDetails.LicenseCategory;
        detailsEntity.SessionName = sessionDetails.SessionName;
        detailsEntity.StartTime = sessionDetails.StartTime;
        detailsEntity.EndTime = sessionDetails.EndTime;
        detailsEntity.CornersPerLap = sessionDetails.CornersPerLap;
        detailsEntity.KmDistPerLap = sessionDetails.KmDistPerLap;
        detailsEntity.MaxWeeks = sessionDetails.MaxWeeks;
        detailsEntity.EventStrengthOfField = sessionDetails.EventStrengthOfField;
        detailsEntity.EventAverageLap = new TimeSpan(sessionDetails.EventAverageLap);
        detailsEntity.EventLapsComplete = sessionDetails.EventLapsComplete;
        detailsEntity.NumCautions = sessionDetails.NumCautions;
        detailsEntity.NumCautionLaps = sessionDetails.NumCautionLaps;
        detailsEntity.NumLeadChanges = sessionDetails.NumLeadChanges;
        detailsEntity.TimeOfDay = sessionDetails.TimeOfDay;
        detailsEntity.DamageModel = sessionDetails.DamageModel;
        detailsEntity.IRTrackId = sessionDetails.IRTrackId;
        detailsEntity.TrackName = sessionDetails.TrackName;
        detailsEntity.ConfigName = sessionDetails.ConfigName;
        detailsEntity.TrackCategoryId = sessionDetails.TrackCategoryId;
        detailsEntity.Category = sessionDetails.Category;
        detailsEntity.WeatherType = sessionDetails.WeatherType;
        detailsEntity.TempUnits = sessionDetails.TempUnits;
        detailsEntity.TempValue = sessionDetails.TempValue;
        detailsEntity.RelHumidity = sessionDetails.RelHumidity;
        detailsEntity.Fog = sessionDetails.Fog;
        detailsEntity.WindDir = sessionDetails.WindDir;
        detailsEntity.WindUnits = sessionDetails.WindUnits;
        detailsEntity.Skies = sessionDetails.Skies;
        detailsEntity.WeatherVarInitial = sessionDetails.WeatherVarInitial;
        detailsEntity.WeatherVarOngoing = sessionDetails.WeatherVarOngoing;
        detailsEntity.SimStartUtcTime  = sessionDetails.SimStartUTCTime;
        detailsEntity.SimStartUtcOffset = new TimeSpan(sessionDetails.SimStartUTCOffset);
        detailsEntity.LeaveMarbles = sessionDetails.LeaveMarbles;
        detailsEntity.PracticeRubber = sessionDetails.PracticeRubber;
        detailsEntity.QualifyRubber = sessionDetails.QualifyRubber;
        detailsEntity.WarmupRubber = sessionDetails.WarmupRubber;
        detailsEntity.RaceRubber = sessionDetails.RaceRubber;
        detailsEntity.PracticeGripCompound = sessionDetails.PracticeGripCompound;
        detailsEntity.QualifyGripCompund = sessionDetails.QualifyGripCompund;
        detailsEntity.WarmupGripCompound = sessionDetails.WarmupGripCompound;
        detailsEntity.RaceGripCompound = sessionDetails.RaceGripCompound;
